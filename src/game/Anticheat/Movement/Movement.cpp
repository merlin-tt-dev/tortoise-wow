...
void Movement::VerifyMovementFlags(uint32 flags, uint32 &removeFlags, bool strict) const
{
    ...
    if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_FLY_HACK))
    {
        ...
        {
            _anticheat->RecordCheatInternal(CHEAT_TYPE_FLY_HACK, "MovementFlags hack: Player %s had MOVEFLAG_SWIMMING and MOVEFLAG_FLYING (flags: 0x%lx)",
                _me->GetName(), flags);
        ...
        {
            _anticheat->RecordCheatInternal(CHEAT_TYPE_FLY_HACK, "MovementFlags hack: Player %s had MOVEFLAG_LEVITATING (only for creatures!) (flags: 0x%lx)",
                _me->GetName(), flags);
        ...
        {
            _anticheat->RecordCheatInternal(CHEAT_TYPE_FLY_HACK, "MovementFlags hack: Player %s had MOVEFLAG_CAN_FLY (flags: 0x%lx)",
                _me->GetName(), flags);
    ...
    {
        _anticheat->RecordCheatInternal(CHEAT_TYPE_FIXED_Z, "MovementFlags hack: Player %s had MOVEFLAG_FIXED_Z (flags: 0x%lx)",
            _me->GetName(), flags);
    ...
    if (sAnticheatConfig.IsEnabled(CHEAT_TYPE_WATER_WALK) &&
        !!(flags & MOVEFLAG_WATERWALKING) && !_me->HasAuraType(SPELL_AURA_WATER_WALK) && !_me->HasAuraType(SPELL_AURA_GHOST) && !pendingWaterWalkRemoval)
    {
...
void Movement::TimeSkipped(const ObjectGuid &mover, uint32 ms)
{
    ...
    sLog.out(LOG_ANTICHEAT_DEBUG, "Time skipped: Player %s (GUID 0x%lx) mover 0x%lx ms %u",
